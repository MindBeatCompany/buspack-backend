...
let FormatServiceRequestValidator = class FormatServiceRequestValidator extends service_request_validator_1.default {
    validateRowData(row, dtv, isFileUploading) {
        ...
        if (homeDelivery.status !== "ok") {
            ...
            if (homeDelivery.value === "NO") {
                ...
                addressNumber = this.validateAddress(row.addressNumber ?
                    ...
                    row.addressNumber, "addressNumber", dtv, isFileUploading);
                addressCpa = this.validateAddress(row.addressCpa ?
                    row.addressCpa.toString() :
                    row.addressCpa, "addressCpa", dtv, isFileUploading);
                ...
                addressFloor = this.validateAddress(row.addressFloor ?
                    ...
                    row.addressFloor, "addressFloor", dtv, isFileUploading);
                addressApartment = this.validateAddress(row.addressApartment ?
                    row.addressApartment.toString() :
                    row.addressApartment, "addressApartment", dtv, isFileUploading);
            }
        }
        ...
        if (isFileUploading) {
            ...
            enabledPlace = sr_field_response_1.srFieldResponse(row.cpa, "ok", "");
            ...
            locality = sr_field_response_1.srFieldResponse(row.locality, "ok", "");
            province = sr_field_response_1.srFieldResponse(row.province, "ok", "");
            cpa = sr_field_response_1.srFieldResponse(row.cpa, "ok", "");
            enabledPlace = sr_field_response_1.srFieldResponse(row.enabledPlace, "ok", "");
        }
    ...
    valueHasValidLength(value, fieldName, dtv) {
        let res = true;
        if (datatype_fields_1.stringFields.includes(fieldName)) {
            ...
            if (value.toString().length > savedLength)
                res = false;
        }
        return res;
    }
    getDefaultValue(fieldName, dtv) {
    ...
    validateValue(value, fieldName, dtv, isFileUploading) {
        let res = sr_field_response_1.srFieldResponse(value, "ok", "");
        ...
        if (datatype_fields_1.numberFields.includes(fieldName)) {
    ...
    validateHomeDelivery(fieldName, value, dtv, isFileUploading) {
        let res = sr_field_response_1.srFieldResponse(value, "ok", "");
        const defaultValue = this.getDefaultValue(fieldName, dtv);
        ...
        if (typeof value === "string")
            value = value.trim();
        if (value === null || value === "")
            return this.setError(res, "Este atributo no puede ser vacío");
    ...
    validateDocNumber(docNumber, dtv, isFileUploading) {
        ...
        return sr_field_response_1.srFieldResponse(docNumber, "ok", "");
    }
    validateEPValue(value, fieldName, dtv) {
        let res = sr_field_response_1.srFieldResponse(value, "ok", "");
        const defaultValue = this.getDefaultValue(fieldName, dtv);
        if (defaultValue) {
            res.value = defaultValue;
    ...
    getGetEpFieldsWithValueNull(locality, province, cpa, epDtv) {
        let res = [];
...
FormatServiceRequestValidator = __decorate([
    common_1.Injectable()
], FormatServiceRequestValidator);
exports.default = FormatServiceRequestValidator;
//# sourceMappingURL=format-service-request-validator.js.map